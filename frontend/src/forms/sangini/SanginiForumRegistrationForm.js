...
const SanginiForumRegistrationForm = () => {
  const [sangName, setSangName] = useState("");
  ...
  const redirectToResponse = (id) => {
    navigate(`/sgn-forum-registraion-response/${id}`, { replace: true });
  };
  ...
  async function FormSubmit(event) {
    ...
    formField.append("sangName", sangName);
    ...
    await axios({
      method: "post",
      url: "http://127.0.0.1:8000/api/forms/sangini/sgnforumregistraion/",
      data: formField,
    })
    ...
    <div>
      <Container>
        <Form onSubmit={FormSubmit}>
          <Row>
            <Col md={8}>
              <h2>Sangini Forum Registration</h2>
            </Col>
          </Row>

          <Row className="mb-3">
            <Form.Group as={Col} controlId="sangName">
              <Form.Label>Group Name</Form.Label>
              <Form.Control
                name="sangName"
                value={sangName}
                onChange={(e) => setSangName(e.target.value)}
              />
            </Form.Group>
...
export default SanginiForumRegistrationForm;
