...
function SanginiCommitteeMeetResponse() {
  const [response, setResponses] = useState([]);
  const responseId = useParams();

  const getResponse = async () => {
    const res = await axios.get(
      `http://127.0.0.1:8000/api/forms/sangini/sgnforumregistraion/${responseId.id}`
    );
...
export default SanginiCommitteeMeetResponse;
