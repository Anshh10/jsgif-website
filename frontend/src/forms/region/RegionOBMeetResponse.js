...
function RegionOBMeetResponse() {
  const [response, setResponses] = useState([]);
  const responseId = useParams();

  const getResponse = async () => {
    const res = await axios.get(
      `http://127.0.0.1:8000/api/forms/region/reg-obmeet/${responseId.id}`
    );
  ...
  const editForm = () => {
    navigate(`/form/robmeet/${response.id}/edit`);
  };
        ...
        <div id="mainForm" className="fResponseBody">
          ...
          <ol className="Lsingle-enter">
            ...
            <li>
              ...
              <span className="response">{response.reporterName}</span> 1 minute
            </li>
          ...
          <ol className="Lsingle-enter">
            ...
            <li>
              ...
              the point(s) in detail to the Chairman and Secretary by email 24
              hours before the date of the meeting.
            </li>
            <li>Total planned duration of the meeting is 30 minutes.</li>
          </ol>
        </div>
      </div>
    </div>
...
export default RegionOBMeetResponse;
