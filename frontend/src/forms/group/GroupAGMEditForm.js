...
const GroupAGMEditForm = () => {
  ...
  const EditForm = () => {
    axios
      .get(`/api/forms/group/grp-agm/${responseId.id}/`)
      .then(function (res) {
        ...
        setSecReportYear(res.data.secReportYear);
  ...
  async function FormSubmit(event) {
    ...
    await axios({
      method: "put",
      url: `/api/forms/group/grp-agm/${response.id}/`,
      data: formField,
    })
        ...
        <Form onSubmit={FormSubmit}>
          ...
          <Row className="mb-3">
            <Form.Group as={Col} controlId="secReportYear">
              <Form.Label>Secretary's For the Year</Form.Label>
              <Form.Control
                placeholder="20XX-XX"
                name="secReportYear"
                value={secReportYear}
...
export default GroupAGMEditForm;
