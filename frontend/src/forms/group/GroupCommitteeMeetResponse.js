...
function GroupCommitteeMeetResponse() {
  const [response, setResponses] = useState([]);
  const responseId = useParams();

  const getResponse = async () => {
    const res = await axios.get(
      `/api/forms/group/grp-committee-meet//${responseId.id}/`
    );
  ...
  const editForm = () => {
    navigate(`/form/grpcommitteemeet/${response.id}/edit`);
  };
      ...
      <div className="centerFormat">
        <div id="mainForm" className="fResponseBody">
          ...
          <ol className="Lsingle-enter">
            ...
            <li>Welcome address by the President 2 minutes.</li>
            <li>
              To read and confirm the minutes of the previous meeting 2 minutes.
            </li>
            <li>Matters arising out of the minutes 2 minutes.</li>
            <li>Secretarial report by Secretary & Jt. Secretary 2 minutes.</li>
            <li>Presentation of events planned. 10 minutes</li>
            ...
            <li>
              To transact any other matter with the permission of the chair 2
              minutes
            </li>
          ...
          <p className="Tdouble-enter">
            For Jain Social Group{" "}
            <span className="response">{response.gName}</span>
          </p>
          <Row>
            <Col className="text-center">
              <p className="Tdouble-enter">
                JSGian <span className="response">{response.grpSecretary}</span>{" "}
              </p>
            ...
            <Col>
              <p className="Tdouble-enter">
                JSGian{" "}
                <span className="response">{response.grpJtSecretary}</span>{" "}
              </p>
          ...
          <ol className="Lsingle-enter">
            ...
            <li>Total planned duration of the meeting is 22 minutes.</li>
          </ol>
        </div>
      </div>
    </div>
...
export default GroupCommitteeMeetResponse;
